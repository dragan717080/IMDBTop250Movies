...
app.get('/genres', async (req, res) => {
    connection.query('SELECT genre_id as genreId, name FROM genres', function (error, results) {
        if (error) throw error;
        res.send(results);
    });
})
...
app.post('/movies', async (req, res) => {
    connection.query(`SELECT * FROM movies WHERE genres REGEXP '${JSON.parse(req.body)}'`, function (error, results) {
        if (error) throw error;
        res.send(results);
    });
})
    ...
    var logos = await Promise.all(
        productionCompanies.map(async (productionCompanyId) => {
            return new Promise((resolve, reject) => {
                connection.query(`SELECT logo FROM production_companies WHERE id = ${productionCompanyId}`, (error, results) => {
                    if (error) reject(error);
                    else {
